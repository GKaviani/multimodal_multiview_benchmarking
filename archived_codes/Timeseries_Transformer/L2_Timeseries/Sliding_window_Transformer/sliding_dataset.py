...
class TimeSeriesDataset(Dataset):
    def __init__(self, directory, mode='train', sampling_rate=400, subseq_length=4, window_stride=2 , num_subseq = 2):
        ...
        self.L1_classes = [cls for cls in os.listdir(self.data_dir) if os.path.isdir(os.path.join(self.data_dir, cls))]

        self.L2_path = [(os.path.join(self.data_dir, activity, folder), activity)
                           for activity in self.L1_classes
                           for folder in os.listdir(os.path.join(self.data_dir, activity))]
        self.L1_classes_name = sorted(self.L1_classes)
        self.files = []
        self.L2_classes = []
        for path , _ in self.L2_path:
            l2_activity =path.split('/')[-1]
            if l2_activity not in self.L2_classes:
                self.L2_classes.append(l2_activity)
            for file in os.listdir(path):
                self.files.append((os.path.join(path , file) , l2_activity))
    ...
    def __getitem__(self, index):
        ...
        return sub_sequences, position_embeddings, sorted(self.L2_classes).index(activity)

    def get_labels(self):
        return [sorted(self.L2_classes).index(activity) for _, activity in self.files]
...
# Example usage:
    data_dir = '/mnt/data-tmp/ghazal/DARai_DATA/timeseries_data/processed6/IMU_RightArm'
    dataset = TimeSeriesDataset(directory=data_dir, mode='train', sampling_rate=400, subseq_length=4, window_stride=2 , num_subseq=2)
