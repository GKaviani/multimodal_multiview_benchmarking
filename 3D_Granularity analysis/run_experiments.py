...
if __name__ == "__main__":
    ...
    save_results_to_json(average_result,
                         os.path.join(args.base_dir ,"results",
                                      f"average seed result_{args.modality}_{args.backbone}_{args.sampling}_ep {args.epochs}_B {args.batch_size}_{args.gl}%_{args.cam_view}_{args.data_dir.split('/')[-1]}_weights {args.weights}.json"))
    # Plotting
    ...
    plt.savefig(os.path.join(args.base_dir ,"figures",
                             f"averaged_training_val_loss_acc_{args.backbone}_{args.sampling}_ep {args.epochs}_B {args.batch_size}_{args.gl}_{args.cam_view}_{args.data_dir.split('/')[-1]}__weights {args.weights}.png"))
    plt.show()
